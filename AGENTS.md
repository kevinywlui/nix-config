...
- **Tier 3 — Low trust, mandatory diff review:** Any input not under `NixOS/` or `nix-community/` on GitHub. Few watchers, high compromise risk. Read the actual code diff before every update — do not skip this. **Current Tier 3 input:** `claude-desktop` (`github:aaddrick/claude-desktop-debian`), a single-maintainer repo that repackages Anthropic's official **Windows** Claude Desktop installer to run on Linux. It is pinned to an explicit commit in `flake.nix` (not a floating branch), so a bump is a deliberate SHA edit — see that input's comment for the full pinning/sandbox rationale. On every bump, the diff review must focus on the build/packaging scripts (`*.sh`), the `app.asar` JS patching and native-binding shim, and the `fetchurl` **installer URL + hash** in the Nix package — a changed URL host is the highest-risk signal.
